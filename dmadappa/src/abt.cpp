...
public:
	static int g_nWaitingForSeq;
}B_globals;
int ReceiverGlobals::g_nWaitingForSeq;
...
void A_output(struct msg message) //ram's comment - students can change the return type of the function from struct to pointers if necessary
{
	...
	nextPacket->seqnum = A_globals.g_nSeqNo;
	for(int i = 0; i< 20; i++) {
		nextPacket->payload[i] = message.data[i];
	}
	//A_globals.g_nNextSeqNo = (A_globals.g_nNextSeqNo + 1) % 2;
	int checksum = GetCheckSum(nextPacket, false);
	nextPacket->checksum = checksum;
	...
	A_globals.g_bMessageInTransit = true;
...
{
	//B's Application layer won't send any messages
}
...
{
	int checksum = GetCheckSum(&packet, true);
	if(checksum != packet.checksum) {
		//Ack is corrupted drop it
		return;
	}
	if(packet.acknum != A_globals.g_nWaitingForAck) {
		//wrong ack ignore it
		return;
	}
	...
	A_globals.g_nSeqNo = (A_globals.g_nSeqNo + 1) % 2;
	A_globals.g_nWaitingForAck = 3;
...
{
	B_transport++;
	//printf("\nReceived trans:%d at B", B_transport);
	if(packet.checksum != GetCheckSum(&packet, false)) {
		//packet corrupted, ignore it
		return;
	}
	if(packet.seqnum != B_globals.g_nWaitingForSeq) {
		//out of order packet received, we have already delivered it up, just ack it
		pkt ack;
	...
	B_globals.g_nWaitingForSeq = (B_globals.g_nWaitingForSeq + 1) % 2;
}
